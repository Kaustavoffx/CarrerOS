...
export function CommunityIntelligence({ profile }: CommunityIntelligenceProps) {
  ...
  const [hoveredCell, setHoveredCell] = useState<{ district: string; category: string; count: number } | null>(null);
  ...
  const fetchHeatmap = useCallback(async () => {
    ...
    try {
      ...
      const data = await response.json();
      setHeatmapData(data.groupedData || null);
      setGapReport(data.report || "");
    } catch (err) {
      console.error(err);
      showToast("Failed to fetch community heatmap analytics.");
    } finally {
  ...
  const heatmapCats = [
    ...
    { key: "mentorship", label: "Mentors" },
    { key: "center", label: "Training Centers" }
  ];

  // Helper to resolve cell HSL colors based on resource counts
  const getCellStyles = (count: number) => {
    ...
    return "bg-cyan-500/15 text-cyan-300 border border-cyan-400/25";
  };

  const getCellStatusText = (count: number) => {
    if (count === 0) return "Severe Shortage (0 active)";
    if (count <= 2) return `Low Density (${count} active)`;
    return `Optimal Access (${count} active)`;
  };
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400 max-w-xl">
              Answer &ldquo;What help exists around me right now?&rdquo; Filter support resources by proximity, analyze eligibility requirements, audit community help shortages, and generate actionable community impact reports.
            </p>
          </div>
      ...
      <div className="flex border-b border-white/5 pb-0">
        ...
        >
          Support Opportunities Directory
        </button>
        <button
          onClick={() => {
          ...
          className={`text-xs font-bold px-4 py-3 border-b-2 transition inline-flex items-center gap-1.5 ${
        ...
        >
          ...
          Community Heatmap Intelligence
        </button>
      </div>
      ...
      {activeSubTab === "directory" ? (
        ...
        /* Community Heatmap Intelligence sub-tab */
        <section className="space-y-6">
          <div className="card-data p-6 rounded-xl relative overflow-hidden">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_bottom_right,rgba(34,211,238,0.03),transparent_50%)] pointer-events-none" />
            
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-3 mb-6 relative z-10">
              <div className="space-y-0.5">
                <h3 className="text-sm font-bold text-white uppercase tracking-wider">Regional Resource Density Heatmap Matrix</h3>
                <p className="text-[11px] text-slate-400">Hover or click cells to view severe support gaps and shortage metrics across districts.</p>
              </div>

              <div className="flex items-center gap-3 text-[9px] uppercase font-bold text-slate-400 shrink-0">
                <span className="inline-flex items-center gap-1">
                  <span className="h-2 w-2 rounded bg-rose-500/30 border border-rose-500/50" />
                  Severe Shortage
                </span>
                <span className="inline-flex items-center gap-1">
                  <span className="h-2 w-2 rounded bg-amber-500/20 border border-amber-500/30" />
                  Low Density
                </span>
                <span className="inline-flex items-center gap-1">
                  <span className="h-2 w-2 rounded bg-cyan-500/20 border border-cyan-400/30" />
                  Optimal Access
                </span>
              </div>
            </div>
            ...
            {loadingHeatmap ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3">
                <Loader2 className="h-8 w-8 text-cyan-400 animate-spin" />
                <p className="text-xs text-slate-400 font-semibold uppercase tracking-widest animate-pulse">Analyzing geographical resource datasets...</p>
              </div>
            ) : !heatmapData ? (
              <p className="text-xs text-slate-500 text-center py-12">Failed to load density counts.</p>
            ) : (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 relative z-10">
                {/* Heatmap Grid Matrix (8 cols) */}
                <div className="lg:col-span-8 overflow-x-auto">
                  <table className="w-full text-left border-collapse min-w-[500px]">
                    <thead>
                      <tr className="border-b border-white/5">
                        <th className="py-2.5 text-[10px] text-slate-500 font-bold uppercase tracking-wider">District / City</th>
                        {heatmapCats.map((cat) => (
                          <th key={cat.key} className="py-2.5 px-4 text-center text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                            {cat.label}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody className="divide-y divide-white/5">
                      {heatmapCities.map((district) => (
                        <tr key={district} className="hover:bg-white/[0.01] transition duration-150">
                          <td className="py-3 text-xs font-bold text-slate-300">{district}</td>
                          {heatmapCats.map((cat) => {
                            const count = heatmapData[district]?.[cat.key] ?? 0;
                            const cellStyle = getCellStyles(count);
                            return (
                              <td key={cat.key} className="py-3 px-4 text-center">
                                <button
                                  onMouseEnter={() => setHoveredCell({ district, category: cat.label, count })}
                                  onMouseLeave={() => setHoveredCell(null)}
                                  className={`px-3 py-1.5 rounded-lg text-xs font-extrabold transition duration-200 w-full min-w-[80px] text-center ${cellStyle}`}
                                >
                                  {count}
                                </button>
                              </td>
                ...
                {/* Tooltip detail / Hover sidebar (4 cols) */}
                <div className="lg:col-span-4 flex flex-col justify-center border border-white/5 bg-white/[0.01] p-5 rounded-xl min-h-[200px]">
                  {hoveredCell ? (
                    <div className="space-y-3">
                      ...
                      <div>
                        <h4 className="text-sm font-bold text-white leading-none">{hoveredCell.district}</h4>
                        <p className="text-xs text-slate-400 mt-1">{hoveredCell.category} Density</p>
                      </div>
                      <div className="bg-white/[0.02] border border-white/5 p-3 rounded-lg">
                        <span className="text-[9px] text-slate-500 font-bold uppercase block">Status Flag</span>
                        <span className={`text-xs font-bold mt-1 block ${
                          hoveredCell.count === 0 ? "text-rose-400" : hoveredCell.count <= 2 ? "text-amber-300" : "text-cyan-300"
                        }`}>
                          {getCellStatusText(hoveredCell.count)}
                        </span>
                        <p className="text-[10px] text-slate-400 mt-1.5 leading-relaxed">
                          {hoveredCell.count === 0 
                            ? "AI Flagged: Severe regional shortage detected. Local candidate pools have zero structured local access in this category."
                            : hoveredCell.count <= 2 
                            ? "AI Flagged: Low density safety warning. Underrepresented local resource capacity. Application queues likely overflowed."
                            : "AI Flagged: Resource coverage is nominal. Adequate channels are open for local enrollment matching."}
                        </p>
                      </div>
                    </div>
                  ) : (
                    <div className="text-center text-slate-500 space-y-2 py-8">
                      <Compass className="h-8 w-8 text-slate-700 mx-auto" />
                      <p className="text-xs">Hover over cells in the heatmap matrix to analyze district gaps and shortage reports.</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          {/* AI generated Help Gap Report document viewer */}
          {gapReport && !loadingHeatmap && (
            <div className="card-purple p-6 rounded-xl space-y-4 relative overflow-hidden">
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(99,102,241,0.02),transparent_50%)] pointer-events-none" />
              
              <div className="flex justify-between items-center border-b border-indigo-400/10 pb-3 relative z-10">
                <div className="flex items-center gap-2">
                  <Brain className="h-4.5 w-4.5 text-indigo-400" />
                  <h3 className="text-xs font-bold text-white uppercase tracking-wider">AI Generated Help Gap Report</h3>
                </div>

                <div className="flex items-center gap-3">
                  <span className="text-[9px] font-bold text-emerald-400 bg-emerald-950/20 border border-emerald-500/10 px-2 py-0.5 rounded-full inline-flex items-center gap-1 uppercase tracking-wider animate-pulse">
                    <span className="h-1 w-1 rounded-full bg-emerald-400" />
                    AI Analyst Online
                  </span>
                  <button
                    onClick={() => copyToClipboard(gapReport, "gap-report")}
                    className="text-indigo-300 hover:text-white text-[10px] font-bold inline-flex items-center gap-1 bg-indigo-950/40 border border-indigo-500/20 px-2 py-1 rounded"
                  >
                    {copiedId === "gap-report" ? <Check className="h-3 w-3" /> : <Copy className="h-3 w-3" />}
                    Copy Report
                  </button>
                </div>
              </div>

              {/* Scrollable markdown document container */}
              <div className="max-h-[450px] overflow-y-auto pr-2 relative z-10 leading-relaxed font-sans text-xs text-slate-300 space-y-4 bg-black/40 border border-white/5 p-5 rounded-xl">
                <pre className="whitespace-pre-wrap font-sans leading-relaxed text-slate-300 text-xs text-left">
                  {gapReport}
                </pre>
              </div>
            </div>
