...
import {
  ...
  Brain, Copy, FileText, Check, ExternalLink, Sliders, ArrowRight
} from "lucide-react";
import type { UserProfileRecord, WorkspaceSnapshotRecord } from "@/lib/supabase/types";
import { CommunityResource, AgentAction } from "@/lib/community-db";
...
export function CommunityIntelligence({ profile, workspace }: CommunityIntelligenceProps) {
  ...
  // Profile AI Matching
  const [aiMatches, setAiMatches] = useState<Array<{ resourceId: string; score: number; reasoning: string }>>([]);
  ...
  const runAiMatching = async () => {
    ...
    try {
      ...
      showToast("Community recommendations updated.");
    } catch (err) {
      console.error(err);
      showToast("AI Matching service unavailable. Using default matching rules.");
    } finally {
      setMatching(false);
    }
  };
  ...
  const triggerAgentAction = async (resourceId: string, resourceName: string, actionType: "verify_eligibility" | "draft_sop_or_application") => {
    ...
    try {
      ...
      // Premium effect: Stream log list with slight delays to show agent thinking
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400 max-w-xl">
              Discover verified regional opportunities, scholarships, free credentials, and emergency help centered exactly around your career goals and current geolocation.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              ...
              {matching ? "Recalibrating..." : "Analyze Matches"}
            </button>
          </div>
        </div>
      </section>

      {/* Primary Coordinates & Proximity Filters Grid */}
      <section className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="card-data p-5 rounded-xl space-y-4 lg:col-span-1">
          ...
          {/* Latitude Longitude Input Controls */}
        ...
        {/* AI matches & suggestions pipeline (3/4 width) */}
        <div className="lg:col-span-3 space-y-6">
          
          {/* AI Verified Matches Panel */}
          <div className="card-purple p-5 rounded-xl">
            <div className="flex items-center justify-between border-b border-indigo-400/10 pb-2 mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold text-white uppercase tracking-wider">AI Verified Matches & Recommendations</h3>
              </div>
            ...
            {matching ? (
              ...
              <div className="text-center py-6">
                <p className="text-xs text-slate-500">No active profile matches computed. Click "Analyze Matches" to evaluate.</p>
              </div>
            ) : (
              <div className="grid gap-3 sm:grid-cols-2">
                {aiMatches.map((match) => {
                  ...
                  return (
                    <div key={match.resourceId} className="border border-indigo-500/10 bg-indigo-950/[0.03] hover:border-indigo-400/20 p-3.5 rounded-xl transition flex flex-col justify-between group">
                      <div>
                        <div className="flex justify-between items-start gap-2 mb-2">
                          <span className="text-xs font-bold text-white group-hover:text-cyan-300 transition line-clamp-1">
                            {opportunity.name}
                          </span>
                          <span className="bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 text-[9px] font-extrabold px-1.5 py-0.5 rounded">
                            {match.score}% Match
                          </span>
                        </div>
                      ...
                      <div className="mt-3.5 pt-2 border-t border-white/5 flex items-center justify-between text-[10px] font-bold">
                        <span className="text-slate-500 uppercase tracking-widest text-[8px]">{opportunity.type}</span>
                        <button
                          onClick={() => triggerAgentAction(opportunity.id, opportunity.name, "verify_eligibility")}
      ...
      {/* Category Filter & Proximity List Section */}
              ...
              return (
                <div key={item.id} className="card-data p-5 rounded-xl flex flex-col justify-between relative overflow-hidden group">
                  <div className="space-y-3">
                    ...
                    <div className="bg-white/[0.01] border border-white/5 p-3 rounded-lg text-[10px]">
                      <span className="text-[8px] text-slate-500 font-bold uppercase tracking-wider block mb-1">Key Eligibility Criteria</span>
                      <ul className="space-y-1 text-slate-300 list-disc list-inside">
                        {Object.entries(item.eligibility).slice(0, 3).map(([key, val]) => (
                          <li key={key} className="truncate">
                            <span className="text-slate-500 capitalize">{key.replace(/_/g, " ")}:</span> {String(val)}
                          </li>
                        ))}
                        {Object.keys(item.eligibility).length === 0 && (
                          <li className="text-slate-500 italic">No restricting eligibility constraints listed.</li>
                        )}
                      </ul>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 pt-3.5 border-t border-white/5 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-1.5">
                      <button
                        onClick={() => triggerAgentAction(item.id, item.name, "verify_eligibility")}
