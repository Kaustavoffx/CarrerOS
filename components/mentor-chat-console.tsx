...
export function MentorChatConsole({ profile, workspace: initialWorkspace }: MentorChatConsoleProps) {
  ...
  useEffect(() => {
    if (MENTOR_LOCKED) {
      setTimeout(() => setLockVisible(true), 100);
    }
  }, []);
    ...
    <div className="min-h-screen text-slate-200 max-w-[1440px] mx-auto relative px-4 sm:px-6">
      ...
      <div className="relative min-h-[70vh] flex flex-col mt-6">
        ...
        <div className={`flex-1 flex flex-col transition-all duration-[400ms] ${
          MENTOR_LOCKED ? "blur-[10px] select-none pointer-events-none" : ""
        }`}>
      ...
      {savedInsights.length > 0 && (
        ...
        {/* Floating Locked Modal Overlay */}
        {MENTOR_LOCKED && (
          <div className={`absolute inset-0 z-40 flex items-center justify-center p-4 bg-black/10 backdrop-blur-[2px] transition-opacity duration-500 ease-out ${
            lockVisible ? "opacity-100" : "opacity-0"
          }`}>
            <div className="liquid-panel w-full max-w-sm rounded-[24px] p-6 text-center border border-cyan-400/20 bg-[#09090b]/90 shadow-[0_12px_48px_rgba(0,0,0,0.8)] relative overflow-hidden flex flex-col items-center gap-5">
              <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-cyan-400/30 to-transparent" />
              
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-cyan-500/25 bg-cyan-500/5 text-cyan-300 text-[9px] font-extrabold tracking-widest uppercase shadow-[0_0_12px_rgba(0,216,255,0.1)]">
                <span className="h-1.5 w-1.5 rounded-full bg-cyan-400 animate-pulse" />
                Private Beta
              </div>

              <div className="relative flex items-center justify-center w-14 h-14 rounded-2xl bg-white/[0.02] border border-white/[0.06] shadow-lg">
                <Image src="/logo.png" alt="CareerOS Logo" width={32} height={32} className="object-contain" />
              </div>

              <div className="space-y-2">
                <h3 className="text-base font-bold text-white uppercase tracking-wider font-geom">AI Mentor Workspace</h3>
                <p className="text-[11px] text-slate-400 leading-relaxed max-w-xs font-semibold">
                  Advanced career coaching, mock simulator boards, and resume reviewer neural pipelines are undergoing active calibration.
                </p>
              </div>

              <div className="w-full border-t border-white/5 pt-4 text-left space-y-2 text-[10px] text-slate-400 font-medium">
                <div className="flex items-center gap-2">
                  <span className="h-1 w-1 bg-cyan-400 rounded-full animate-pulse" />
                  <span>Coach Simulator & STAR Drills</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="h-1 w-1 bg-cyan-400 rounded-full animate-pulse" />
                  <span>Resume Keyword & ATS Semantic Alignment</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="h-1 w-1 bg-cyan-400 rounded-full animate-pulse" />
                  <span>Interactive System Design reviews</span>
                </div>
              </div>

              <div className="w-full bg-cyan-500/5 border border-cyan-400/15 rounded-xl px-4 py-2.5 text-[10px] text-cyan-300 font-bold tracking-wide flex items-center justify-center gap-2">
