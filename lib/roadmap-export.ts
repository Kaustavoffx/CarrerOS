...
export async function generateRoadmapPdfBlob(report: RoadmapPdfReport) {
  ...
  function drawWatermark() {
    ...
    doc.text("CAREEROS", centerX, pageHeight / 2 - 20, { align: "center", angle: 45 });
    doc.setFontSize(14);
    doc.text("CAREER INTELLIGENCE SYSTEM", centerX, pageHeight / 2 + 20, { align: "center", angle: 45 });
    doc.restoreGraphicsState();
  }
  ...
  function drawPageFrame(pageNum: number, totalPagesCount: number) {
    ...
    // Header Left: CareerOS Monogram + text
    doc.setFillColor("#00D8FF");
    doc.circle(margins.left, 28, 3, "F");
    doc.setTextColor("#FFFFFF");
    doc.text("CareerOS", margins.left + 8, 31);
    
    // Header Right: Page-specific identifier
    let pageIdText = "CAREER SNAPSHOT";
    if (pageNum === 2) pageIdText = "MARKET INTELLIGENCE";
    else if (pageNum >= 3 && pageNum < 2 + sprintsData.length + 1) {
      pageIdText = `SPRINT ${String(pageNum - 2).padStart(2, "0")} DOSSIER`;
    } else {
      pageIdText = "RECRUITER READY DASHBOARD";
    }
    ...
    // Footer Left
    doc.setTextColor("#64748b");
    doc.text("CareerOS Confidential", margins.left, pageHeight - 24);
    
    // Footer Center
    doc.text("Generated via CareerOS Intelligence Engine", centerX, pageHeight - 24, { align: "center" });
  ...
  // Logo Monogram Cover
  doc.setFillColor("#00D8FF");
  doc.circle(margins.left, 48, 4, "F");
  doc.setFillColor("#3B82F6");
  doc.circle(margins.left + 7, 48, 3.2, "F");
  doc.setFont("CMGeom", "normal");
  doc.setFontSize(10);
  doc.setTextColor("#FFFFFF");
  doc.text("CareerOS", margins.left + 14, 51);
